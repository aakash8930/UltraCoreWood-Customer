...
export default function CheckoutPage() {
  ...
  const totalDiscount = cart.items.reduce((sum, i) => sum + (((i.product.price * (i.product.discount || 0)) / 100) * i.quantity), 0);
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
        ...
        const orderPayload = {
            ...
            paymentBreakdown: { itemsTotal: subtotal, tax, shipping: deliveryFee, discount: totalDiscount, total: totalAmount },
        };
      ...
      <div className="checkout-content">
        <div className="checkout-left">
          <section className="shipping-address">
            ...
            {loadingAddresses ? <div className="loading-container">Loading Addresses...</div> :
              (addresses.length > 0 && !showAddressForm ? renderAddressSelector() : renderNewAddressForm())
            }
          </section>
        ...
        <div className="checkout-right">
          <div className="order-summary">
            ...
            <div className="price-breakdown">
                ...
                <div className="price-row savings"><span>Savings</span><span>-{formatPrice(totalDiscount)}</span></div>
                <hr/>
                <div className="price-row"><span>Subtotal</span><span>{formatPrice(subtotal)}</span></div>
                <div className="price-row"><span>Tax</span><span>{formatPrice(tax)}</span></div>
                <div className="price-row"><span>Delivery</span><span>{deliveryFee > 0 ? formatPrice(deliveryFee) : 'FREE'}</span></div>
                <hr/>
                <div className="price-row total-amount"><span>Total amount</span><span>{formatPrice(totalAmount)}</span></div>
            </div>
